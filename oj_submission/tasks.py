...
@shared_task
def judge(task_id, case_id, test_case_config, subcheck_config, lang, code, limit):
    submission = Submission.objects.get(id=task_id)
    judger = JudgeClient()
    submission.status = StatusChoices.JUDGING
    submission.save(update_fields=['status'])
    result = judger.judge(task_id, case_id, test_case_config, subcheck_config, lang, code, limit)
